...
Git是一种快捷的拉取和推送远程仓库的一种选择，你也可以使用其他的拉取和推送的软件,但是我还是推介你使用Git因为他足够便捷和已经在大部分IDE中已经内置了这个功能。如何下载Git请访问下面网站选择自己的版本
...
Git的安装请参考本片文章[(26条消息) Git 详细安装教程（详解 Git 安装过程的每一个步骤）_mukes的博客-CSDN博客_git安装](https://blog.csdn.net/mukes/article/details/115693833)这篇文章可以辅助你知道怎么在安装中选择应该的选项。

### 注册账号和邮箱

使用Git之前都需要注册自己的账号和密码以此来知道是谁推送了分支以我的D盘为例子在D盘中的任意空白位置右键选择GitBash或者直接在桌面选择GitBash如果没有GitBash选项那么就在Git的安装目录中打开并选择git-bash.exe

以下为例子以自己的安装目录为准：

![image-20221028101936567](./git%E5%9F%BA%E6%9C%AC%E6%95%99%E7%A8%8B.assets/image-20221028101936567.png)

选择git-bash.exe后会探出一个小窗git的小窗的操作累计类似于vim所以是不能使用Windows上的快捷键的否则可能会输入的乱码行为

进行小窗后先进行账号的初始化命令为**git config --选项 信息 “(输入基本信息)”**
...
这里就不做测试了这样我们就完成了账号和邮箱的本地初始化

**注：以上的操作只是对本地进行了初始化如果要进行远程的推送还是需要登录远程服务器的账号和密码，以远程服务器的为准**
...
我以下面文件夹为例子你也可以打开自己随便一个文件夹为例

> D:\Dev-Text

![image-20221028103737010](./git%E5%9F%BA%E6%9C%AC%E6%95%99%E7%A8%8B.assets/image-20221028103737010.png)

这样我们就初始化了一个Git的基本文件同时文件夹中会创建一个.git文件他是用来存放我们每次提交,保存的指针

![image-20221028103848761](./git%E5%9F%BA%E6%9C%AC%E6%95%99%E7%A8%8B.assets/image-20221028103848761.png)

如果没有看见这个文件夹,那么请参考[怎么显示win10系统隐藏文件夹 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/259761453#:~:text=1、点击打开Win10系统的文件资源管理器，点击菜单栏上的“查看”。 2、找到并点击“选项”。,3、在打开的文件夹选项界面中点击“查看”选项卡。 4、将高级设置中找到并取消勾选“隐藏受保护的操作系统文件”，然后选择“显示隐藏的文件，文件夹或驱动器”。)或者[Win11如何查看隐藏文件？Win11查看隐藏文件的方法_当客下载站 (downkr.com)](https://www.downkr.com/news/228630_1.html#:~:text=Win11查看隐藏文件的方法 1、在磁盘中打开任意文件夹，并点击下图箭头所指的工具选项。,2、在下拉页中打开显示页面。 3、勾选隐藏的项目按钮即可显示查看私密文件。)
...
一般来说在不加附加命令的时候在git-bash下输入git commit就会进入vim一样控制面板的模式如果已经知道vim怎么键盘操作的可以使用该命令或者可以参考这篇文章[vim命令大全 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/61515833).
...
并且输入git status查看状态时没有其他选项的时候就可以进行提交到远程仓库了
...
这样提交和添加都会一起到暂存区但是我不推荐你这样做因为这就意味这你这整个文件夹都会被进行提交

### Git分支

在介绍Git的提交之前我们先介绍Git的分支系统,**在进行Git的分支之前请确保你至少已经进行了一次暂存区的提交**否则在暂存区没有数据的时候Git是不允许你建立新的分支的
...
详细原理可以查看:[Git 分支管理 | 菜鸟教程 (runoob.com)](https://www.runoob.com/git/git-branch.html)
...
一般来说我们都是克隆别人的仓库来使用和更改那么怎么使用克隆别人的远程仓库呢命令如下:
...
我们需要一个文件夹来使用我是以D:\Git-Text为存放的路径可以随自己而定

![image-20221028120100226](./git%E5%9F%BA%E6%9C%AC%E6%95%99%E7%A8%8B.assets/image-20221028120100226.png)

我们已经克隆完该文件夹一般克隆的时候都会把其的.git全部克隆下来所以就不需要进行初始化只需要进行[提交](#提交到本地暂存区)和[添加](#添加到本地暂存区)

我们可以通过git remote -v命令查看已经有的远程仓库
...
每次提交之前请确保已经完成了[初始化](#初始化)的工作这样我们就已经完成了对于远程仓库的拉取和推送
