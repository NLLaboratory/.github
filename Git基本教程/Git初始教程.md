...
一般来说在不加附加命令的时候在git-bash下输入git commit就会进入vim一样控制面板的模式，如果已经知道vim怎么操作的可以使用该命令或者可以参考这篇文章[vim命令大全 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/61515833).
...
例如我想我想提交到本地暂存区并且提交的时候直接提交不使用vim的而且提交的名字为First那么就输入
...
这样提交和添加都会一起到暂存区但是我不推荐你这样做因为这就意味这你这整个文件夹都会被进行提交和添加。
